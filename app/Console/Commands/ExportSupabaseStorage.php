...
class ExportSupabaseStorage extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            $files = $this->getAllStorageFiles($supabaseUrl, $serviceKey, $bucket);
    ...
    private function uploadZipToSupabase($zipPath, $filename)
    {
        try {
            ...
            if (!isset($result['Key']) || !$result['Key']) {
                throw new \Exception('Erreur upload ZIP: ' . json_encode($result));
            }

            $this->info('☁️  Upload ZIP réussi !');
            ...
            $this->error('❌ Erreur upload ZIP : ' . $e->getMessage());
